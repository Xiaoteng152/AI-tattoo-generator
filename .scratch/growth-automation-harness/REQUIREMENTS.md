# 需求文档：Growth Automation Harness
...
1. 运营配置产品方向、关键词、数据源、过滤条件、AI Prompt、输出模板和调度。
...
MVP 要证明数据提取、数据清洗、AI 增强和运营配置可以组成完整增长闭环，而不是做一个泛化低代码平台。
...
- Connector 暴露小接口：接收 Workflow Config 中的数据源配置，返回 Raw Item 和抓取元数据。
- 第一个 Connector 选择 Reddit，因为它最适合从帖子和评论里发现用户痛点。
- 第二个 Connector 选择 Etsy 或 Pinterest。Etsy 更适合验证商业需求，Pinterest 更适合验证视觉趋势。
- 第一个演示场景固定为 AI tattoo generator。
- 默认关键词包括 `ai tattoo`、`tattoo generator`、`minimal tattoo`、`fine line tattoo`、`tattoo ideas`、`custom tattoo design`。
...
- Feedback Signal 要提前设计，但 MVP 可以先支持手动录入指标或 CSV 导入。
- 系统不应该变成通用低代码自动化平台。它的领域是海外 C 端产品的增长发现和增长执行。
- 线上 MVP 推荐使用 Vercel + Next.js Route Handlers 承载 UI 和轻量 API，使用 Supabase Postgres 承载 Prisma 管理的业务数据。
- Supabase 在本阶段只作为托管 PostgreSQL 使用，不强制切换到 Supabase Auth；当前 Auth.js Google OAuth 可以继续服务 DeepSearch 和写操作保护。
- Vercel 函数不承载常驻 worker。Workflow 和 DeepSearch 先以小批量同步运行为主，超过平台运行边界后再引入独立 worker 或托管队列。
...
- 官方 **X API v2 recent search** 需要 Developer 账户 **credits**；未充值时会返回 `402 Payment Required`。
- 当前 MVP **不依赖充值**。Workflow 会优先尝试 X API；遇到 `401/402/403` 时 **fallback 到 SoPilot RSS**，且只保留与关键词匹配的帖子。
- SoPilot 热帖不一定覆盖 tattoo 垂类；本地演示可先用 **Bitcoin / 中文热点** 等预设关键词验证链路。
- **未实现项（待充值后启用）**：
  - X API 按关键词 recent search
  - 分页抓取（`next_token`）超过 SoPilot 单页 RSS 上限
  - 基于 X 官方 engagement 字段的精确排序
- 充值与 credits 管理属于 **Billing / 运维配置**，不在当前 MVP 代码范围内。

### Workflow 配置简化（已实现方向）

- Dashboard 不再展示固定的 seed Workflow Config 标签墙。
- 用户在运行前 **自行选择预设或输入关键词**，再触发 X Workflow。
- 后台仍保留最小 `WorkflowConfig` 记录以满足 Prisma 关系和 source 配置（当前仅 `twitter`）。

## 面试叙事

这个项目应该被包装成增长工程系统，而不是普通爬虫或 Prompt 套壳。
